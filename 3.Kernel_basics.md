...
``` sh
# 安装编译工具
sudo apt-get install build-essential libtool autotools-dev automake pkg-config zip bison
...
# 程序会提示输入123选择架构、cpu类型等信息，按提示选择三次1即可
# 1. v7 ARM Architecture
# 1. SDK_Debug_Defconfigs
# 1. ti_sdk_am3x_debug

# 初始化Linux内核配置
make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- ti_sdk_am3x_debug_defconfig
